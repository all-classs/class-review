...
public class UserClassListService {
    ...
    public void save(Enrollment enrollment){
        userClassListDataRepository.save(enrollment);
    }

    @Transactional(readOnly = true)
    public List<UserClassListResponse> findClassForSemester(int userNumber){

        List<Enrollment> list = userClassListDataRepository.findByUserNumberWithClassAndUser(userNumber);
        if(list.isEmpty()){
    ...
    public List<MyPageStudentResponse> myPageWithStudent(int userNumber){
        ...
        List<Enrollment> list = userClassListDataRepository.findByUserNumber(user).orElseThrow(() -> new ClassNotFoundException("해당 학생이 수강한 강의가 없습니다."));
    ...
    public Enrollment findByUserNumber(int userNumber, String lectureName){
        ...
        Enrollment list = userClassListDataRepository.findByUserNumberAndLecture(user, lecture).orElseThrow(() -> new NoPermissionReviewException(""));
